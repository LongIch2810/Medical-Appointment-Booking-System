...
|---|---|---|
| `users` (`User`) | `username`/`email` unique not-null, `phone` unique nullable, `password` nullable (null = tài khoản Google-only), `fullname`, `gender` bool default true, `date_of_birth`, `picture`, `address`, `isAdmin` bool default false (`[UNCERTAIN]`/khả năng dead — không thấy guard/service nào đọc cờ này ngoài RBAC roles), `is_active` default false, `is_locking` default false | 1:1 `Doctor`, `UserSetting`; 1:N `UserRole`, `Notification`, `Article` (author), `Conversation`, `Otp`, `Message` (sender), `ChannelMembers`, `Relative`, `Appointment` (booker), `AuditLog`, `Complaint`. Soft delete. |
