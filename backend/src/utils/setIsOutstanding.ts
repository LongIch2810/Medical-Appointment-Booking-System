...
export const calculateFinalScore = (
  ...
  const finalScore =
    0.7 * avg_rating + 0.3 * Math.log(1 + appointments_completed);
  return finalScore;
};

export const setIsOutstandingDoctor = (
...
export const setIsOutstandingDoctors = (doctors: DoctorOutstanding[]) => {
  return doctors.map((doctor: DoctorOutstanding) => ({
    ...doctor,
