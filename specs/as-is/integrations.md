...
The chatbot uses `@langchain/langgraph-checkpoint-postgres` with a dedicated PostgreSQL role (`chatbot_report_assistant`) and schema (`langgraph`). Migration `1788800000000-setupLangGraphPersistence.ts` provisions the isolated schema/role; migration `1788900000000-renameLangGraphRole.ts` additively renames the legacy role so already-migrated databases are handled safely. The role is restricted to its LangGraph schema; neither migration changes `chatbot_readonly` or grants access to business tables. The migration runner must be authorized to create/alter the role and create/change ownership of the `langgraph` schema. The role password is configured as `LANGGRAPH_DB_PASSWORD` in both backend and chatbot environments (at least 16 characters); `LANGGRAPH_DB_USER` must be `chatbot_report_assistant`. Both reuse `DB_HOST`, `DB_PORT`, and `DB_NAME`.
...
Patient booking is a two-phase operation in the LangGraph workflow: `booking_appointment_tool` proposes but does not create an appointment, `interrupt()` pauses for explicit UI approval, and `Command({ resume })` is the only route to the commit node. The backend validates ownership and the latest approval message, then forwards its stored operation UUID and summary. The appointment API enforces that UUID with a unique index and returns the same appointment on retry without repeating success notifications. A normal user message while the graph is still waiting for approval resumes as `REVISE` and does not call the appointment endpoint. Deleting a patient conversation deletes its checkpoint before soft-deleting the business transcript; preferences are intentionally cross-conversation and remain.
