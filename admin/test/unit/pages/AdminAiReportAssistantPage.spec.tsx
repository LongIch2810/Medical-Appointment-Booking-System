...
function detail(messages: ReportAssistantConversationDetailResponse["messages"] = []): ReportAssistantConversationDetailResponse {
  return {
    conversation: {
      id: 5,
...
function setDefaultHooks(
  ...
  const create = vi.fn().mockResolvedValue({ data: { conversation: { id: 5 } } });
  const send = vi.fn().mockResolvedValue({});
  const confirm = vi.fn().mockResolvedValue({});
  const loadOlder = vi.fn().mockResolvedValue({ data: { messages: [], nextBeforeMessageId: null } });
  hookMocks.useReportAssistantConversations.mockReturnValue({
    data: { data: { conversations: includeConversation ? [detail().conversation] : [], total: Number(includeConversation), page: 1, limit: 20, totalPages: Number(includeConversation) } },
    isLoading: false,
    isError: false,
    refetch: vi.fn(),
  });
  hookMocks.useReportAssistantConversation.mockImplementation((id: number | null) => ({
    data: id === 5 ? { data: detail(messages) } : undefined,
    isLoading: false,
    isError: false,
    refetch: vi.fn(),
  }));
  hookMocks.useCreateReportAssistantConversation.mockReturnValue({ mutateAsync: create, reset: vi.fn(), isPending: false, error: null });
  hookMocks.useSendReportAssistantMessage.mockReturnValue({ mutateAsync: send, reset: vi.fn(), isPending: false, error: null });
  hookMocks.useConfirmReportAssistantPlan.mockReturnValue({ mutateAsync: confirm, reset: vi.fn(), isPending: false, error: null });
  hookMocks.useLoadOlderReportAssistantMessages.mockReturnValue({ mutateAsync: loadOlder, reset: vi.fn(), isPending: false, error: null });
...
describe("AdminAiReportAssistantPage", () => {
  it("starts a conversation from a Vietnamese quick prompt", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /So sánh số lịch hẹn theo chuyên khoa/ }));
  ...
  it("sends a follow-up with Enter and keeps Shift+Enter for a newline", async () => {
    ...
    fireEvent.change(composer, { target: { value: "So sánh với tháng trước" } });
    fireEvent.keyDown(composer, { key: "Enter", shiftKey: true });
    expect(send).not.toHaveBeenCalled();
    expect(composer).toHaveValue("So sánh với tháng trước");

    fireEvent.keyDown(composer, { key: "Enter" });
    await waitFor(() => expect(send).toHaveBeenCalledWith({ id: 5, message: "So sánh với tháng trước" }));
  });
  ...
  it("shows an explicit confirm action only for the latest proposed plan", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Xác nhận và tạo báo cáo" }));
  ...
  it("loads older conversation messages by cursor", async () => {
    ...
    hookMocks.useReportAssistantConversation.mockImplementation(() => ({
      data: { data: {
        ...
        messages: [{ id: 50, role: "ASSISTANT", action: "ANSWER", content: "Recent response", plan: null, report: null, createdAt: "2026-08-20T10:00:00Z" }],
    ...
    await user.click(screen.getByRole("button", { name: "Tải tin nhắn cũ hơn" }));
  ...
  it("renders a report preview for a generated report and does not show a plan action for refusal", () => {
    ...
    expect(screen.queryByRole("button", { name: "Xác nhận và tạo báo cáo" })).not.toBeInTheDocument();
    ...
    expect(screen.getByText("Dữ liệu doanh thu hiện chưa được cung cấp.")).toBeInTheDocument();
