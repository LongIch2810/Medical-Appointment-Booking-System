    ...
    return (
      <div className="container mx-auto px-4 py-6 mt-16 md:mt-28">
        <ErrorState
          ...
          description={t("doctor.notFoundDesc")}
          onRetry={() => refetch()}
        />
      </div>
    );
          ...
          <div className="flex-1 text-center sm:text-left space-y-2.5 min-w-0">
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
              <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-slate-900 dark:text-slate-100">
                {t("doctor.drPrefix")} {doctor.fullname}
              </h2>
              {doctor.doctor_level && (
            ...
            <p className="text-sm sm:text-base font-semibold text-primary flex items-center justify-center sm:justify-start gap-2">
              ...
              <span>{doctor.specialty?.name || t("doctor.generalSpecialist")}</span>
            </p>
            ...
            <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 flex items-center justify-center sm:justify-start gap-2">
              ...
              <span>{doctor.workplace || t("doctor.generalHospital")}</span>
            </p>
            ...
            <div className="pt-1">
              ...
              >
                ...
                {t("doctor.experienceYearsCount", { years: doctor.experience })}
              </Badge>
            </div>
          </div>
        </div>
          ...
          <section className="space-y-3">
            ...
            <ul className="text-xs sm:text-sm text-slate-700 dark:text-slate-300 space-y-3.5">
              <li className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t("doctor.phoneLabel")}</p>
                  <p className="font-semibold text-slate-800 dark:text-slate-200">{doctor.phone || t("doctor.notUpdated")}</p>
                </div>
              </li>
              <li className="flex items-start gap-3">
                <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-primary/10 text-primary shrink-0">
                ...
                <div>
                  <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t("doctor.emailLabel")}</p>
                  <p className="font-semibold text-slate-800 dark:text-slate-200">{doctor.email || t("doctor.notUpdated")}</p>
                </div>
              </li>
              <li className="flex items-start gap-3">
                <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-primary/10 text-primary shrink-0">
                ...
                <div>
                  <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t("doctor.workAddress")}</p>
                  <p className="font-semibold text-slate-800 dark:text-slate-200">{doctor.address || doctor.workplace || t("doctor.notUpdated")}</p>
                </div>
              </li>
            </ul>
          </section>
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 sm:p-8 shadow-xs space-y-6">
        <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
          <h3 className="text-base sm:text-lg font-bold text-slate-900 dark:text-slate-100">
            {t("doctor.detailSchedule")}
          </h3>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            {t("doctor.selectDatePrompt")}
          </p>
        </div>
          ...
          <div className="flex-1 space-y-4">
            <div className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/60 p-3.5">
              <h4 className="text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-200">
                {t("doctor.scheduleDayTitle", { date: formatDate(selectedDate, dateLocale) })}
              </h4>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                {t("doctor.schedulesCount", { count: selectedDaySchedules.length })}
              </p>
            </div>
            ...
            {selectedDaySchedules.length > 0 &&
              !checkSchedulesExpireOrBooked(
                ...
                <div className="flex justify-center pt-2">
                  ...
                  >
                    {isPending ? <Loading /> : t("doctor.continueBookingBtn")}
                  </Button>
                </div>
              )}
          </div>
